...
if (strtoupper($Sign) != strtoupper(md5($WebID.$Key.$tradeNo.$Money.$title.$memo))){
		...
		*/
		...
		$mysql_username		=	"a0122145834"; 		// 连接数据库用户名
		$mysql_password		=	"68626936";				// 连接数据库密码
		$mysql_database		=	"a0122145834"; 			// 数据库的名字
		...
		if ($mysql_conn){
			...
			*/
			$rs=mysql_query("Select * From income Where order_sn='$memo'");	//将查询sql语句的结果存到$rs变量中

			$num=mysql_num_rows($rs);
														//mysql_num_rows函数的作用就是返回记录笔数.就是你的数据表中的总笔数
			$user_id=0;
			if($num>0){
			...
			if ($UserNameIsExist==true){		/*如果用户名存在，就自动发货*/
				...
				**********更新数据库事例开始*********************************************************
				...
				mysql_query("Update users set money=money+".($Money)." Where id='$user_id'");
				/* 增加充值记录 */
				mysql_query("update income set pay_status=1 where order_sn='$memo' and income_money=$Money ");
				$sql="select * from users where id=$user_id";
				$rs=mysql_query($sql);
				$rows=mysql_fetch_assoc($rs);
				if($rows['agencies_id']!=0)
				{
						mysql_query("Update agencies set income=income+".($Money)." Where id=".$rows['agencies_id']);
				}
				...
				echo "ordersn does not exist!";	//当用户名不存在时，就提示此信息，并且不会自动发货
			}
			//*******************************************************************
